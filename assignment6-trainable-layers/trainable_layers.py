...
def define_dense_model_with_hidden_layers(input_length, 
    ...
    model.add(keras.layers.Dense(hidden_layers_sizes[0], input_shape=(input_length,), activation=activation_func_array[0]))
    model.add(keras.layers.Dense(hidden_layers_sizes[1], activation=activation_func_array[1]))
...
def set_layers_to_trainable(model, trainable_layer_numbers):
    ...
    for i in trainable_layer_numbers:
